...
interface AppContextType {
  ...
  addSubscriber: (subscriber: Omit<Subscriber, "id" | "createdAt" | "lastResetDate">) => void
...
export function AppProvider({ children }: { children: ReactNode }) {
  const [products, setProducts, productsLoaded] = useLocalStorage<Product[]>("products", [])
  const [sales, setSales, salesLoaded] = useLocalStorage<Sale[]>("sales", [])
  const [paymentReminder, setPaymentReminder, reminderLoaded] = useLocalStorage<PaymentReminder | null>(
    "paymentReminder",
    null,
  )
  const [subscribers, setSubscribers, subscribersLoaded] = useLocalStorage<Subscriber[]>("subscribers", [])

  const isLoaded = productsLoaded && salesLoaded && reminderLoaded && subscribersLoaded
  ...
  const addSubscriber = (subscriber: Omit<Subscriber, "id" | "createdAt" | "lastResetDate">) => {
  ...
  const updateSubscriber = (id: string, updatedSubscriber: Partial<Subscriber>) => {
    setSubscribers(subscribers.map((s) => (s.id === id ? { ...s, ...updatedSubscriber } : s)))
  }
